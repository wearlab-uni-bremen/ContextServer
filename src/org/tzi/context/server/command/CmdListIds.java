...
public class CmdListIds implements ServerCommand {

	//@Override
	public ServerCommandResult processCommand(ServerCommandParameters scp) {
		ServerCommandResult scr = new ServerCommandResult();
		scr.reply.append(scp.csi.getUsedIds().size());
		for(Integer uid : scp.csi.getUsedIds()) {
			scr.reply.append(" ");
			scr.reply.append(uid.toString());
		}
		return scr;
	}
